$(document).ready(function () {
    try {
        let tipo = '';
        let rutaBase = 'http://localhost:8888/';
        const regex = /&lt;br&gt;/gi;
        const arryAreas = [];
        ...
        let contador = 0;
        for (let i = 0; i < arryAreas.length; i++) {
        ...
        const descripcion = tipo.descripcion.replace(regex, '\n');
...
//get name
        $("#InputEmail1").blur(function () {
            const str = $("#InputEmail1").val();
            if(str!=null && str!==""){
                const ruta = rutaBase + "getNameByEmail/" + str;
                $.get(ruta,
                function (res) {
                    $("#InputName").val(res);
                });
